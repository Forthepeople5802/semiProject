	 ...
	 if(con3==true){
		 ...
		 console.log("dd");
		 	startPage =data.startPage;
		 		...
		 		main+='<tr><td>'+json.list[i].customerId+'</td>';
		 		main+='<td>'+decodeURIComponent(json.list[i].customerName)+'</td>';
		 		main+='<td>'+json.list[i].customerEmail+'</td>';
		 		main+='<td style="float:right;"><button type="button" onclick="withdraw(\''+json.list[i].customerId+'\');" class="btn btn-outline-primary" >강퇴</button></td></tr>';
		 		}
		 ...
		 location.href = "http://localhost:7771/food/views/adminMy/admin_my.jsp";
...
<div>
	<ul class="pagination" >
	<li class="page-item"><h1>사업자 미승인 회원</h1></li>
...
<input type="text" name="seValue" style="vertical-align:top; ">&nbsp;<input type="submit" value="검색" style="background:green; vertical-align:top; border:green; color:white; height:21.5px; text-align:center "></form>
	...
	if(request.getAttribute("list") != null){
		...
		<%}else{%>
		<tr style = "background-color: white;">
		<td align="center" colspan="5"><%=request.getAttribute("noSearchArpproval") %></td>
		</tr>
		<% } 
...
<h2>&emsp;축제 정보 수정</h2>
		...
		<form action="/food/fes_update" method="post"
			...
			<!-- 축제 연락처 -->
			<strong>축제 고객센터</strong>&emsp;&emsp;&emsp;&emsp; <input type="text"
...
<div>
	<ul class="pagination" >
	<li class="page-item"><h1>사업자 승인거절 회원</h1></li>
	
	</ul></div>
	
	
	<h4 align="center">거절 회원목록 수: </h4>
