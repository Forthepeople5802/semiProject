 ...
 */
@WebServlet("/businessmy_test")
public class BusinessPageModServlet_Test extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public BusinessPageModServlet_Test() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			truck_Live_OnOff= multi.getParameter("truckOnOff");
			...
			String menuFileTagName[] = multi.getParameterValues("mFile");
			new_menu_img=new String[menuFileTagName.length];
			menuName = multi.getParameterValues("mName");
			menuPrice = new int[multi.getParameterValues("mPrice").length];
			for(int i=0; i<multi.getParameterValues("mPrice").length;i++)
			{
				menuPrice[i]=Integer.parseInt(  multi.getParameterValues("mPrice")[i] );
			}
			...
			for (int i = 0; i < menuFileTagName.length; i++) {
				uploadFile = multi.getFilesystemName(menuFileTagName[i]);
				...
				} else if( menuFileTagName[i].contains("oriMenuFile")    ){
					if(multi.getParameter(menuFileTagName[i]+"Img")!=null)
					new_menu_img[i]=multi.getParameter(menuFileTagName[i]+"Img");
					else
			...
			{	menu[i]=new Menu();
				...
				if(menuInsert_result>0)
					System.out.println((i+1)+"번째 메뉴 등록 성공");
			}
