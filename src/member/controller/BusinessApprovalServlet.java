...
/**
 * Servlet implementation class BusinessApprovalServlet
 */
@WebServlet("/bapprov")
public class BusinessApprovalServlet extends HttpServlet {
     ...
     */
    public BusinessApprovalServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//전체 목록 갯수 조회
		int listCount = mservice.getListApprovalCount();
		
		//현재 페이지에 출력할 목록 조회
		ArrayList<Business> list = mservice.selectApproval(currentPage, limit);
		...
		if(list.size()>0) {
			...
			if(request.getParameter("apvbtnMessage")!=null)
		request.setAttribute("apvbtnMessage", request.getParameter("apvbtnMessage"));
