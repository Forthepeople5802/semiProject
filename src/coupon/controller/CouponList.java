...
public class CouponList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int listCount = 0;
		listCount = cservice.getCouponCount(userId);

		// 한 페이지당 출력할 목록 갯수 지정
		int limit = 10;
		ArrayList<Coupon_l> clist = cservice.getCouponList(userId,currentPage,limit);
		...
		if(clist.size() > 0) {
			...
			view= request.getRequestDispatcher("views/userMy/user_my.jsp");
			request.setAttribute("tab", "tab-4");
			request.setAttribute("couponMessage", "쿠폰없음");
			view.forward(request, response);
		}
